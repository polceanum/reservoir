...
def initialize_weights_sparse(resSize, inSize, inInterSize, density, rho=None):
    Win = (torch.rand(inInterSize, 1 + inSize, dtype=torch.float32) - 0.5) * 1
...
def run_reservoir_sparse(data, Win, W_sparse, trainLen, initLen, resSize, a):
    ...
    Win_full = torch.concatenate([Win, torch.zeros((resSize-Win.shape[0], Win.shape[1]))])
    X = torch.zeros((1 + inSize + resSize, trainLen - initLen), dtype=torch.float32)
    x = torch.zeros((resSize, 1), dtype=torch.float32)
    for t in range(trainLen):
        u = data[t]
        x = (1 - a) * x + a * torch.tanh(Win_full @ torch.vstack((torch.tensor([1.0], dtype=torch.float32), u.view(1, 1))) + W_sparse @ x)
        if t >= initLen:
...
def run_generative_mode(data, model, Win, W, testLen, trainLen, a, initial_x):
    ...
    for t in range(testLen):
        x = (1 - a) * x + a * torch.tanh(Win_full @ torch.vstack((torch.tensor([1.0], dtype=torch.float32), u.view(1, 1))) + W @ x)
...
def main():
    ...
    Win, W = initialize_weights_sparse(resSize, inSize, inInterSize, density, rho=estimate_rho(resSize, density))
